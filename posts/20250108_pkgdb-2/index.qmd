...
chaines <- pkgdb$`Authors@R` |> 
  map(gsub, pattern = "(\\s+|[\"])+", replace = "")
chaines[[2]]
...
# unique(pkgdb$Version) |> grepl(pattern = "^0[.]")
# pkgdb[grepl(pkgdb$Version, pattern = "^0[.]"), "Version"]
# 
# unique(gsub(pkgdb$Version, pattern = "\\.[0-9.-]*$", replacement = ""))
# 
# res <- pkgdb$Version |> regexpr(pattern = "^([0-9-]*)")
# resultat <- pkgdb$Version |>  regmatches(res)
# resultat
# resultat <- gsub(pattern = "\\.[0-9.-]*$", x = pkgdb$Version, replacement = "")

# pattern = "(^[0-9-]*)"
# 
# match_regex <- function(x, pattern) {
#   
#   return(regmatches(x = x, regexpr(pattern = pattern, text = x)))
# 
# }
# 
# match_regex_all <- function(x, pattern) {
#   
#   return(regmatches(x = x, gregexpr(pattern = pattern, text = x)))
# 
# }
# 
# text = pkgs$`Authors@R`[pkgs$Package == "aberrance"]
# texte1 <- stringi::stri_extract_all(text, regex = "[[:alnum:][@]-.]+")[[1]]
# texte1
