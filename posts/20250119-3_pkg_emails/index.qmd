...
#### puis en un tibble 
  ...
  - `tbl_all_emails_in_authorsR` avec tous les emails, i.e doublons compris

```{r}
#| label: tbl_all_emails_in_authorsR

tbl_all_emails_in_authorsR <- as_tibble(mat_emails_in_authorsR) |> 
  rename(Package = V1, 
         Version = V2,
         Depends = V3,
        email_id = V4, 
...
```
  ...
  - `tbl_unique_emails_in_authorsR` après suppression des emails doublons
...
#### résumé par ligne du data.frame `pdb`
  
::: {.callout-important collapse="false"}

Dans la colonne `Authors@R`

  - **`emails_count`** donne le nombre total d'adresses de messagerie
  (même si ces dernières sont dupliquées)
...
#| 
summary_emails_per_pdb_row <- tbl_all_emails_in_authorsR |> 
...
#### récupération des lignes du data.frame `pdb` où il y a un problème
...
pkg_with_max_difference <- bad_emails_count_rows |> 
  filter(difference == max(difference)) |> 
  pull(Package)

pkg_with_max_difference

```

Exemple pour le paquet **`r pkg_with_max_difference`** :

```{r}

select_bad_row(pkg = pkg_with_max_difference)
...
mail_tlds <- tbl_unique_emails_in_authorsR$email_domain |> 
